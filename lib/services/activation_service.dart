...
/// 2. [activateDesktop] — call from [ActivationScreen] for first-time setup.
/// 3. [verifyActivation] — call at startup (after loading) to confirm the
...
class ActivationService {
  ...
  Future<void> loadPersistedActivation() async {
    ...
    final accessToken = await DatabaseService.instance.getAppMeta(_kAccessToken);
    ...
    DatabaseSchema.setActivatedTenant(businessId: businessId, branchId: branchId);
  }
  ...
  Future<ActivationResponse> activateDesktop({
    ...
    if (data == null) throw Exception('Empty activation response from server.');
  ...
  Future<bool> verifyActivation() async {
    ...
    try {
      ...
      if (e.response?.statusCode == 401) {
        ...
        if (kDebugMode) debugPrint('ActivationService: token revoked by server (401 on verify)');
      }
      return false;
    } catch (_) {
      return false;
  ...
  Future<void> refreshActivationToken() async {
    final storedRefreshToken =
        await DatabaseService.instance.getAppMeta(_kRefreshToken);
  ...
  Future<void> revokeActivation() async {
    ...
    // Reset sync timestamps and cursor so a future activation starts fresh.
    ...
    if (kDebugMode) debugPrint('ActivationService: activation revoked — tokens and sync metadata cleared');
  }
  ...
  Future<void> _persistActivation(ActivationResponse r) async {
    ...
    if (r.refreshToken != null) {
      await DatabaseService.instance.setAppMeta(_kRefreshToken, r.refreshToken!);
    }
    if (r.licenseExpiresAt != null) {
      await DatabaseService.instance.setAppMeta(_kLicenseExpiresAt, r.licenseExpiresAt!);
    }
