...
// App scaffold background (#F7F8F6) — visible until Flutter paints the first frame.
static HBRUSH g_window_background_brush = nullptr;

using EnableNonClientDpiScaling = BOOL __stdcall(HWND hwnd);

void EnsureWindowBackgroundBrush() {
  if (!g_window_background_brush) {
    g_window_background_brush = CreateSolidBrush(RGB(247, 248, 246));
  }
}
...
const wchar_t* WindowClassRegistrar::GetWindowClass() {
  if (!class_registered_) {
    ...
    window_class.hIcon =
        LoadIcon(window_class.hInstance,
                 MAKEINTRESOURCE(IDI_APP_ICON));

    EnsureWindowBackgroundBrush();
...
void Win32Window::ResizeChildToClient() {
  if (!window_handle_ || child_content_ == nullptr) {
    return;
  }
...
bool Win32Window::Show() {
  ...
  ResizeChildToClient();
  const bool shown = ShowWindow(window_handle_, SW_SHOW) != 0;
...
void Win32Window::ApplyLockedBounds(HWND hwnd) {
  ...
  // Do not use SWP_SHOWWINDOW here — WM_SIZE must not reveal the window before
  // Flutter's first frame (Show() handles visibility).
  SetWindowPos(hwnd, HWND_TOPMOST, locked_x_, locked_y_, locked_width_,
               locked_height_, SWP_NOACTIVATE | SWP_FRAMECHANGED);
}
